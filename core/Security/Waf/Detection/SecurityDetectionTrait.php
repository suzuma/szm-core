...
namespace Core\Security\Waf\Detection;
...
trait SecurityDetectionTrait
{
     ...
     */
    protected function xss(string $v): bool
    {
        return (bool)preg_match(
            '/(<script[\s>]|<\/script|javascript\s*:|vbscript\s*:|'
            . 'on\w+\s*=|alert\s*\(|confirm\s*\(|prompt\s*\(|'
            . 'document\s*\.\s*(cookie|write|location|domain)|'
            . 'eval\s*\(|setTimeout\s*\(|setInterval\s*\(|'
            . 'innerHTML\s*=|outerHTML\s*=|insertAdjacentHTML|'
            . 'String\.fromCharCode|window\.location|'
            . '<svg[^>]*on\w+|<iframe|<object|<embed|<base\s)/i',
            $v
        );
    }
     ...
     *         - 0 o false si no se detecta.
     */
    protected function path_traversal(string $v): bool
    {
     ...
     *         - 0 o false si no se detecta.
     */
    protected function command_injection(string $v): bool
    {
        return (bool)preg_match(
            '/\b(cat|ls|whoami|pwd|curl|wget|bash|sh|dash|zsh|'
            . 'python[23]?|perl|ruby|php|node|nc|netcat|ncat|'
            . 'chmod|chown|rm|mv|cp|find|kill|ps|env|export|'
            . 'echo|printf|tee|dd|tar|gzip|base64|xxd|hexdump)\b'
            . '.*[;\|`>]/i',
            $v
        );
    }
    ...
    protected function open_redirect(string $v): bool
    {
        ...
        // El dominio propio se excluye leyendo APP_URL para que funcione
        // en cualquier proyecto sin modificar el core.
        $appHost    = parse_url($_ENV['APP_URL'] ?? '', PHP_URL_HOST) ?? '';
        $ownDomain  = $appHost ? preg_quote($appHost, '/') : 'localhost';

        return (bool)preg_match(
            '/(?:^|[\?&=\s])(?:https?:)?\/\/(?!(?:www\.)?' . $ownDomain . ')[a-z0-9\-\.]+\.[a-z]{2,}/i',
            $v
        );
    }
